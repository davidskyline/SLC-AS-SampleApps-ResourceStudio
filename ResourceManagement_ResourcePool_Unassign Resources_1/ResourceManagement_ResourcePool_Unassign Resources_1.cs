...
namespace Script
{
	...
	public class Script
	{
		...
		private static List<ResourceDataMapping> UnassignResourcesFromPool(SrmHelpers srmHelpers, Guid poolId, List<ResourceDataMapping> resourceMappings)
		{
			...
			foreach (var resourceMapping in resourceMappings)
			{
				...
				resourceMapping.Resource.PoolGUIDs.Remove(poolId);
		...
		private void RunSafe()
		{
			...
			var updatedResources = UnassignResourcesFromPool(srmHelpers, resourcePoolData.PoolId, existingResources);

			var resourcesDataToUpdate = resourcesData.Except(existingResources.Select(x => x.ResourceData)).ToList();
			resourcesDataToUpdate.AddRange(updatedResources.Select(x => x.ResourceData));
